...
class ROSImageListenNRenderer:
    ...
    def init_sensor(self, sensor_type, sensor_options):
        if sensor_type == 'ROSSubimage':
            return self.node.create_subscription(Image, sensor_options['ros_topic'], self.save_ros_image, 10)
        else:
            return None
...
def run_input_node(args):
    ...
    # FIX: replace create_rate + rate.sleep() with plain time.sleep().
    # create_rate depends on /clock when use_sim_time=True, which causes
    # rate.sleep() to block indefinitely if no /clock is being published.
    dt = 1.0 / FREQ
    ...
    ROSImageListenNRenderer(display_man, 'ROSSubimage', {'ros_topic': '/carla_node/cam_down/image_raw'},
                            display_pos=[0, 2], node=node)
    ROSImageListenNRenderer(display_man, 'ROSSubimage', {'ros_topic': '/yolo_node/sort_mot_frame'},
                            display_pos=[0, 0], node=node)
    ROSImageListenNRenderer(display_man, 'ROSSubimage', {'ros_topic': '/yolo_node/yolo_pred_frame'},
                            display_pos=[0, 1], node=node)
    map_listen_renderer = ROSImageListenNRenderer(display_man, 'ROSSubimage',
                            {'ros_topic': '/path_planner/map_image'},
                            display_pos=[0, 0], node=node, display_scale=2)
    ...
    while rclpy.ok():
        ...
        # FIX: use time.sleep instead of rate.sleep()
        _sleep(dt)
...
def main():
    argparser = argparse.ArgumentParser(description='ROS DISPLAY NODE')
    argparser.add_argument(
        '--res',
        metavar='WIDTHxHEIGHT',
        default='1600x800',
