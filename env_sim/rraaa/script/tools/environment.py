...
#         adv_obj = self.world.get_blueprint_library().filter("vehicle.uli.uli_cora")[0]
...
#         # Rosbag path
#         bag_path = os.path.join(self.object_dir, "recorded_topics")
...
#         rotation = carla.Rotation(rad2deg(-pitch), rad2deg(-yaw), rad2deg(roll))
...
#         settings.no_rendering_mode = bool(getattr(args, 'no_rendering', False))
...
#         self.sub_jax_guam_pose = self.node.create_subscription(
#             PoseStamped, f'/{self.vehicle_type}/pose', self.callback_jax_guam_pose, 10)
...
#         ### Local PyGame display ###
#         local_display_cfg = self.config.get('local_display', {})
#         self.enable_local_display = bool(local_display_cfg.get('enabled', True))
#         self.display_width  = int(local_display_cfg.get('width',  getattr(self.args, 'width',  1280)))
#         self.display_height = int(local_display_cfg.get('height', getattr(self.args, 'height', 720)))
#         self.display_layout = local_display_cfg.get('layout', 'overview_front')
#         self.screen = None
#         self.display_font = None
#         self._display_empty_frame_count = 0
#         self._init_local_display()
...
#     def _init_local_display(self):
#         if not self.enable_local_display:
#             log.info("Local pygame display is disabled by config.")
#             return
#         try:
...
#             )
#             pygame.display.set_caption("CARLA env_sim viewer")
#             self.display_font = pygame.font.SysFont("monospace", 18)
#             log.info(f"Initialized local pygame display at {self.display_width}x{self.display_height}.")
#         except pygame.error as e:
#             self.enable_local_display = False
#             self.screen = None
#             self.display_font = None
#             log.error(f"Failed to initialize local pygame display: {e}")

#     def _handle_display_events(self):
#         if not self.enable_local_display or self.screen is None:
#             return
#         for event in pygame.event.get():
#             if event.type == pygame.QUIT:
#                 log.info("Pygame window closed by user.")
#                 rclpy.shutdown()
#                 return
#             if event.type == pygame.KEYUP and event.key in (pygame.K_ESCAPE, pygame.K_q):
#                 log.info("Pygame viewer requested shutdown.")
#                 rclpy.shutdown()
#                 return

#     def _camera_data_to_surface(self, camera_data):
#         if camera_data is None:
#             return None
#         try:
#             if hasattr(camera_data, 'raw_data') and hasattr(camera_data, 'width') and hasattr(camera_data, 'height'):
...
#             log.error(f"Failed to convert camera frame to pygame surface: {e}")
#             return None

#     def _draw_letterboxed(self, surface, rect):
#         if self.screen is None or surface is None:
#             return
...
#         overview_data = getattr(getattr(self, 'camera_overview', None), 'data', None)
#         front_data    = getattr(getattr(self, 'camera_front',    None), 'data', None)

#         overview_surface = self._camera_data_to_surface(overview_data)
#         front_surface    = self._camera_data_to_surface(front_data)

#         if self.display_layout == 'front_only':
#             main_surface = front_surface or overview_surface
#             self._draw_letterboxed(main_surface, self.screen.get_rect())
#         elif self.display_layout == 'overview_only':
#             main_surface = overview_surface or front_surface
#             self._draw_letterboxed(main_surface, self.screen.get_rect())
#         else:
#             left_rect = pygame.Rect(
#                 pad, pad,
#                 max(1, self.display_width // 2 - int(1.5 * pad)),
#                 self.display_height - 2 * pad,
#             )
#             right_rect = pygame.Rect(
#                 self.display_width // 2 + pad // 2, pad,
#                 self.display_width - (self.display_width // 2 + int(1.5 * pad)),
#                 self.display_height - 2 * pad,
#             )
#             self._draw_letterboxed(overview_surface or front_surface, left_rect)
#             self._draw_letterboxed(front_surface or overview_surface, right_rect)

#         if overview_surface is None and front_surface is None:
#             self._display_empty_frame_count += 1
#             if self.display_font is not None:
#                 msg = self.display_font.render("No camera frame received yet", True, (255, 255, 255))
#                 msg_rect = msg.get_rect(center=(self.display_width // 2, self.display_height // 2))
...
#         if self.display_font is not None:
#             overlay_lines = [
#                 f"server_fps={self.fps_timer.server_fps:.1f}",
#                 f"client_fps={self.client_clock.get_fps():.1f}",
#                 f"ego={getattr(getattr(self, 'ego_vehicle', None), 'type_id', 'n/a')}",
#             ]
#             for i, line in enumerate(overlay_lines):
#                 text = self.display_font.render(line, True, (255, 255, 255))
#                 self.screen.blit(text, (10, 10 + i * 20))

#         pygame.display.flip()

#     def _warmup_camera_streams(self, max_ticks=20):
#         """Tick a few frames so attached cameras have time to deliver the first image."""
#         needed = ['camera_front', 'camera_overview']
#         for _ in range(max_ticks):
#             ready = all(getattr(getattr(self, name, None), 'data', None) is not None for name in needed)
#             if ready:
#                 return True
#             try:
#                 if self.args.asynch:
...
#             except Exception:
#                 pass
#             self.render_local_display()
#         return all(getattr(getattr(self, name, None), 'data', None) is not None for name in needed)
...
#     def tick(self):
#         ### Advance CARLA first so sensors publish fresh frames for this loop ###
#         if self.args.asynch:
#             self.world.wait_for_tick()
#         else:
#             self.world.tick()

#         # FIX: check collision panic flag in the main thread (safe to act on here).
#         # The CollisionSensor callback runs in a CARLA C++ thread and cannot raise
#         # directly — it sets panic_triggered instead, and we handle it here.
#         if getattr(self.collision_sensor_ego, 'panic_triggered', False):
#             msg = self.collision_sensor_ego.panic_event
#             log.warning(f"[COLLISION PANIC] {msg}")
#             self.collision_sensor_ego.panic_triggered = False
#             self.collision_sensor_ego.panic_event = None
#             # Uncomment the line below if you want a hard stop on collision:
#             # raise RuntimeError(f"Collision detected: {msg}")
...
#             adv_obj,
#             carla.Transform(
#                 location=carla.Location(x=x_location, y=y_location, z=z_location),
#                 rotation=carla.Rotation(roll=roll_angle, pitch=pitch_angle, yaw=yaw_angle)
#             )
#         )
#         adv_obj.set_target_velocity(velocity=carla.Vector3D(x_vel, y_vel, z_vel))
#         adv_obj.set_autopilot(False)
#         adv_obj.set_enable_gravity(False)
...
#         # FIX: skip complex objects whose records directory does not exist
#         # instead of crashing with AssertionError.
#         records_dir = self.config['adv_objects']['complex_objects']['records_dir']
#         self.complex_objects = []
#         for obj_name in self.config['adv_objects']['complex_objects']['object_names']:
#             obj_dir = os.path.join(records_dir, obj_name, 'tmp')
#             if not os.path.exists(obj_dir):
#                 log.warning(f"Complex object '{obj_name}' records dir not found, skipping: {obj_dir}")
#                 continue
#             self.complex_objects.append(
#                 ComplexObject(self.world, records_dir, obj_name, self.node)
#             )
...
#         self.camera_front    = SensorManager(self.world, 'RGBCamera',
#             carla.Transform(carla.Location(x=2, z=1.5),   carla.Rotation(yaw=+00, pitch=-10)),
#             self.ego_vehicle, {'fov': '90.0', 'image_size_x': '400', 'image_size_y': '400'})
#         self.camera_left     = SensorManager(self.world, 'RGBCamera',
#             carla.Transform(carla.Location(x=0, z=2.4),   carla.Rotation(yaw=-90)),
#             self.ego_vehicle, {'fov': '90.0', 'image_size_x': '400', 'image_size_y': '400'})
#         self.camera_right    = SensorManager(self.world, 'RGBCamera',
#             carla.Transform(carla.Location(x=0, z=2.4),   carla.Rotation(yaw=+90)),
#             self.ego_vehicle, {'fov': '90.0', 'image_size_x': '400', 'image_size_y': '400'})
#         self.camera_back     = SensorManager(self.world, 'RGBCamera',
#             carla.Transform(carla.Location(x=-2, z=1.5),  carla.Rotation(yaw=+180, pitch=-10)),
#             self.ego_vehicle, {'fov': '90.0', 'image_size_x': '400', 'image_size_y': '400'})
#         self.camera_down     = SensorManager(self.world, 'RGBCamera',
...
#         self.camera_up       = SensorManager(self.world, 'RGBCamera',
#             carla.Transform(carla.Location(x=0, z=2.4),   carla.Rotation(pitch=90)),
#             self.ego_vehicle, {'fov': '90.0', 'image_size_x': '400', 'image_size_y': '400'})
#         self.camera_overview = SensorManager(self.world, 'RGBCamera',
...
#         lidar_common = {
#             'channels': '64', 'range': '100', 'points_per_second': '230400',
#             'upper_fov': '-27', 'lower_fov': '-90', 'rotation_frequency': '10',
#             'horizontal_fov': '360', 'sensor_tick': '0.1',
#         }
#         self.lidar_sensor_down    = SensorManager(self.world, 'LiDAR', self.transform_lidar_from_vehicle_down,    self.ego_vehicle, lidar_common)
#         self.lidar_sensor_up      = SensorManager(self.world, 'LiDAR', self.transform_lidar_from_vehicle_up,      self.ego_vehicle, lidar_common)
#         self.lidar_sensor_left    = SensorManager(self.world, 'LiDAR', self.transform_lidar_from_vehicle_left,    self.ego_vehicle, lidar_common)
#         self.lidar_sensor_right   = SensorManager(self.world, 'LiDAR', self.transform_lidar_from_vehicle_right,   self.ego_vehicle, lidar_common)
#         self.lidar_sensor_back    = SensorManager(self.world, 'LiDAR', self.transform_lidar_from_vehicle_back,    self.ego_vehicle, lidar_common)
#         self.lidar_sensor_forward = SensorManager(self.world, 'LiDAR', self.transform_lidar_from_vehicle_forward, self.ego_vehicle, lidar_common)

#         self.transform_lidar_from_vehicle = carla.Transform(carla.Location(x=0, y=0, z=0), carla.Rotation(pitch=0, yaw=0, roll=0))
#         self.lidar_sensor = SensorManager(self.world, 'LiDAR', self.transform_lidar_from_vehicle, self.ego_vehicle, lidar_common)

#         self.collision_sensor_ego = CollisionSensor(self.ego_vehicle, panic=True)

#         # Warm up sensor streams so the first pygame frames are not blank.
#         for _ in range(3):
#             if self.args.asynch:
#                 self.world.wait_for_tick()
#             else:
...
#         ### Override the pose, i.e., transform ###
...
#         multiarray_ros_msg = pack_multiarray_ros_msg(
#             self.msg_mat.mat, state_val_np, ','.join(veh_list_str), ','.join(state_key))
...
#         # FIX: synchronous_master must match the world's synchronous mode.
#         # Using False in synchronous mode causes apply_batch_sync to not wait
#         # for tick confirmation, leading to inconsistent spawn state.
#         synchronous_master = not self.args.asynch
...
#         try:
#             if self.camera_front:    self.camera_front.sensor.destroy()
#             if self.camera_left:     self.camera_left.sensor.destroy()
#             if self.camera_right:    self.camera_right.sensor.destroy()
#             if self.camera_back:     self.camera_back.sensor.destroy()
#             if self.camera_up:       self.camera_up.sensor.destroy()
#             if self.camera_down:     self.camera_down.sensor.destroy()
#             if self.camera_overview: self.camera_overview.sensor.destroy()
#             if self.lidar_sensor:    self.lidar_sensor.sensor.destroy()
#             if self.collision_sensor_ego: self.collision_sensor_ego.sensor.destroy()
#         except Exception as e:
...
class Environment():
    ...
    def tick(self):
        ...
        if getattr(self.collision_sensor_ego, 'panic_triggered', False):
            ...
            self.collision_sensor_ego.panic_triggered = False
            self.collision_sensor_ego.panic_event = None
            # Uncomment to hard-stop on collision:
            # raise RuntimeError(f"Collision detected: {msg}")
