...
class MapImage(object):
    ...
    def draw_road_map(self, map_surface, carla_world, carla_map, world_to_pixel, world_to_pixel_width):
        ...
        # Find and Drwa Static Vehicles
        vehicle_bboxes = carla_world.get_level_bbs(carla.CityObjectLabel.Car)
        for boundbox in vehicle_bboxes:
            boundbox_transform = carla.Transform(boundbox.location, boundbox.rotation)
            bb = boundbox.extent
            corners = [carla.Location(x=-bb.x, y=-bb.y),
                         carla.Location(x=bb.x, y=-bb.y),
                         carla.Location(x=bb.x, y=bb.y),
                         carla.Location(x=-bb.x, y=bb.y)]
            boundbox_transform.transform(corners)
            corners = [world_to_pixel(p) for p in corners]
            pygame.draw.polygon(map_surface, COLOR_SKY_BLUE_0, corners)
